...
public class FansActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        //显示订阅的
        subscribeAdapter = new SubscribeAdapter(FansActivity.this);
        lRecyclerViewAdapter = new LRecyclerViewAdapter(subscribeAdapter);

        DividerDecoration divider = new DividerDecoration.Builder(FansActivity.this)
    ...
    private void setOnListener() {
        ...
        lrecyclerView.setOnLoadMoreListener(new OnLoadMoreListener() {
            ...
            public void onLoadMore() {
                page++;
                isLoadmore=true;
                boolean connected = NetUtils.isConnected(FansActivity.this);
                if(connected){
                    initData();
                }else{
                    pb_loading.setVisibility(View.GONE);
                    //提示网不给力，但是还是有视频页面
                    Toast.makeText(FansActivity.this, "网络不给力，请检查网络设置", Toast.LENGTH_SHORT).show();
    ...
    private void initData() {
        ...
        if(!status.equals("1")){
            ...
            OkHttpUtils.get().url(Constants.GETFANS+page).addHeader("Authorization",""+"Bearer "+token).build().execute(new StringCallback() {
                @Override
                ...
                public void onResponse(String response, int id) {
                    ...
                    if(count==0){
                        ...
                        if(isLoadmore){
                            Toast.makeText(FansActivity.this, "没有更多数据", Toast.LENGTH_SHORT).show();
                            lrecyclerView.refreshComplete(20);
                            lRecyclerViewAdapter.notifyDataSetChanged();
                        }else{
                            //显示没有订阅的
